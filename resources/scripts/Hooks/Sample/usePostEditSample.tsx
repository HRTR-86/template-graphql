...
export interface PostEditSampleParameter {
  data: PostEditSampleData;
  only: string[];
  handleSuccess: (page: Page) => void;
  handleError: (errors: Errors) => void;
}

const usePostEditSample = () => {
  const loadingContext = useLoadingContext();

  function postEditSample({
    data,
    only,
    handleSuccess,
    handleError,
  }: PostEditSampleParameter): void {
    router.reload({
      method: 'post',
      data: {
  ...
  return { postEditSample };
};

export default usePostEditSample;
